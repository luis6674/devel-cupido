$(function () {
  ...
  function openWindow(id) {
    ...
    // Lazy-load deferred images and videos on first open
    $win.find('img[data-src]').each(function () {
      $(this).attr('src', $(this).data('src')).removeAttr('data-src');
    });
    $win.find('video[data-src]').each(function () {
      this.src = $(this).data('src');
      $(this).removeAttr('data-src');
      this.load();
    });
    $win.find('iframe[data-src]').each(function () {
      this.src = $(this).data('src');
      $(this).removeAttr('data-src');
    });

    // Bands in Town widget script: load only when Tour window first opens
    if (id === 'concerts' && !bitWidgetLoaded) {
      bitWidgetLoaded = true;
      $('<script>', {
        charset: 'utf-8',
    ...
    // intl-tel-input: load only when the newsletter window first opens,
    // regardless of which entry point (icon, dock, lock-screen link) opened it
    if (id === 'newsletter' && !itiLoaded) {
      itiLoaded = true;
      $('<link>', {
        rel: 'stylesheet',
  ...
  $('#newsletter-form').on('submit', function (e) {
    ...
    // City — not empty, within maxlength
    const $city = $('#field_city');
    if (!$city.val().trim() || $city.val().trim().length > 60) fail($city);
  ...
  $('#survey-form').on('submit', function (e) {
    ...
    // Sanitise text inputs
    $(form).find('input[type="text"], input[type="email"], textarea').val(function (_, v) {
      return $.trim(v.replace(/(<([^>]+)>)/gi, ''));
    });
    ...
    // Email — valid format, only required field besides consent
    const $email = $('#survey_field_email_address');
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test($email.val())) fail($email);
    ...
    $.ajax({
      ...
      xhrFields: { withCredentials: false },
      success: function () {
        $('#survey_form_container').fadeOut(function () {
          $('#survey_form_response').fadeIn();
        });
      },
      error: function () {
        alert('Ha ocurrido un error. Por favor, inténtalo más tarde.');
  ...
  // Encuesta pop-up: auto-show once per page load
  setTimeout(function () {
    openWindow('survey');
  }, 2500);
